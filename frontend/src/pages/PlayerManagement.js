  ...
  return (
    <div className="p-8 bg-gradient-to-br from-white to-gray-50 min-h-screen overflow-y-auto flex flex-col">
      <div className="flex justify-between items-center mb-8 bg-white backdrop-blur-xl rounded-lg p-6 shadow-lg border border-gray-300">
        <div>
          <h1 className="text-3xl font-bold bg-gradient-to-r from-red-600 to-red-600 bg-clip-text text-transparent">Player Management</h1>
          <p className="text-gray-700 mt-2">Manage and register tournament players</p>
        </div>
        <button
          onClick={() => {
            setShowModal(true);
      ...
      <div className="mb-6">
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-white border border-gray-300 text-gray-900 placeholder-gray-500 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-600 focus:border-transparent"
          />
        </div>
      </div>

      {/* Players Table */}
      <div className="bg-white backdrop-blur-xl rounded-lg shadow-lg border border-gray-300 overflow-y-auto flex-1 flex flex-col">
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-300">
              {filteredPlayers.map((player) => (
                <tr key={player._id} className="hover:bg-gray-100 transition-colors bg-gray-50">
                  <td className="px-6 py-4 text-sm font-medium text-red-600">{player.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{player.email}</td>
                  <td className="px-6 py-4 text-sm text-gray-600">{player.phone || '-'}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{player.stats.totalMatches}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{player.stats.winPercentage}%</td>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
          <div className="bg-white backdrop-blur-xl rounded-lg shadow-2xl p-8 max-w-md w-full border border-gray-300">
            <h2 className="text-2xl font-bold bg-gradient-to-r from-red-600 to-red-600 bg-clip-text text-transparent mb-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-4 pt-4">
                <button
                  type="submit"
                  className="flex-1 bg-gradient-to-r from-red-600 to-red-600 text-white py-2 rounded-lg hover:from-red-700 hover:to-red-700 transition-all"
                >
                ...
                <button
                  ...
                  className="flex-1 bg-gray-200 text-gray-700 py-2 rounded-lg hover:bg-gray-300 transition-all border border-gray-300"
                >
                  Cancel
                </button>
              </div>
